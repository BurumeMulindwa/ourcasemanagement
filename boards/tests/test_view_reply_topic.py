...
class ReplyTopicTestCase(TestCase):
    ...
    '''
    def setUp(self):
        self.board = Board.objects.create(reference='SCP.I.234576', subject='investigations',
                      query='Automatic Exchange of Information.')
        self.username = 'john'
        self.password = '123'
        user = User.objects.create_user(username=self.username, email='john@doe.com', password=self.password)
        self.topic = Topic.objects.create(subject='Hello, world', board=self.board, starter=user)
        Post.objects.create(notes='Lorem ipsum dolor sit amet', topic=self.topic, created_by=user)
