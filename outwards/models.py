...
class Outward(models.Model):
    ...
    def get_updates_count(self):
        return Update.objects.filter(case__outward=self).count()

    def get_last_update(self):
        return Update.objects.filter(case__outward=self).order_by('-created_at').first()
...
class Case(models.Model):
    ...
    outward = models.ForeignKey(Outward, related_name='topics', on_delete=models.CASCADE)
    initiator = models.ForeignKey(User, related_name='cases', on_delete=models.CASCADE, default=1)
    nature_of_information_requested = MultiSelectField(choices=information_choices, default=A)
    nature_of_information_received = MultiSelectField(choices=information_choices, default=A)
...
class Update(models.Model):
    ...
    case = models.ForeignKey(Case, related_name='updates', on_delete=models.CASCADE)
